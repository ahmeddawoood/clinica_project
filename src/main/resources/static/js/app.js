...
document.querySelectorAll('form[data-confirm]').forEach(form => {
        form.addEventListener('submit', e => {
            if (!confirm(form.getAttribute('data-confirm') || 'Ești sigur?')) {
                e.preventDefault();
            }
        });
    });
    ...
    document.querySelectorAll('[data-confirm]:not(form)').forEach(el => {
        el.addEventListener('click', e => {
            if (!confirm(el.getAttribute('data-confirm') || 'Ești sigur?')) {
                e.preventDefault();
            }
        });
    });
...
const apptDateInput = document.getElementById('appointmentDate');
    if (apptDateInput) {
        ...
        apptDateInput.addEventListener('change', () => {
            ...
            if (hour < 8 || hour >= 18) {
                apptDateInput.setCustomValidity('Programarile se pot face doar între 08:00 și 18:00.');
